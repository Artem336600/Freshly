    ...
    try:
        ...
        chat_response = client.chat.complete(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "Ты — эксперт в подборе продуктов питания. Твоя задача — составить список продуктов, которые идеально соответствуют запросу пользователя, используя доступные продукты из базы данных. Старайся предлагать разнообразие в категориях (овощи, мясо, фрукты и т.д.). Выводить тиы должен только сами продукты, без доп информации(продукты находятся между **). Выводишь только продукты их всегда должно быть 20 в столбик, без ** нумеруя их "
                },
                {
                    "role": "user",
